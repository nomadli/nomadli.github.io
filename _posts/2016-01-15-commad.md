...
sysctl -w net.ipv4.ip_forward=1 修改某个内核参数
...
- vi /etc/systemd/journald.conf && systemctl restart systemd-journald
    - Storage=persistent 手动创建/var/log/journal会永久保存,否则/run/log/journal
...
- gateway
- ftp
- 查看HTTP GET 请求 tcpdump -s 0 -A 'tcp dst port 80 and tcp[((tcp[12:1] & 0xf0) >> 2):4] = 0x47455420'
- 查看HTTP POST 请求 tcpdump -s 0 -A 'tcp dst port 80 and (tcp[((tcp[12:1] & 0xf0) >> 2):4] = 0x504f5354)'
- 查看请求响应头及body tcpdump -A -s 0 'tcp port 80 and (((ip[2:2] - ((ip[0]&0xf)<<2)) - ((tcp[12]&0xf0)>>2)) != 0)' || tcpdump -X -s 0 'tcp port 80 and (((ip[2:2] - ((ip[0]&0xf)<<2)) - ((tcp[12]&0xf0)>>2)) != 0)'
- ip[2:2] > 576
...
31. hdiutil create -o /x/x -size 8G -volname dname -layout SPUD -fs HFS+J
32. hdiutil attach /x/x.dmg -noverify -mountpoint /Volumes/dname
33. sudo /x/Install\ macOS\ Mojave.app/Contents/Resources/createinstallmedia --volume /Volumes/dname --nointeraction
34. hdiutil detach /volumes/Install\ macOS\ Mojave
35. hdiutil convert /x/x.dmg -format UDTO -o /x/x.cdr(iso格式)
36. curl https://mesu.apple.com/assets/macos/com_apple_macOSIPSW/com_apple_macOSIPSW.xml |grep ipsw
