...
ssh -C 压缩传输，提高传输速度。
    ...
    -i 私有文件path
    -N 建立静默连接（建立了连接但看不到具体会话）纯隧道模式
    -g 允许远程主机连接本地用于转发的端口。
    -L 本地端口转发 c->local->ip2->ip ssh -L 本地端口:目标IP:目标端口 usr@ip2
    -R 远程端口转发 c->ip->local ssh -R 80:localhost:2979 usr@ip
    -D 动态转发（SOCKS代理）ssh -D 1080 usr@ip AllowTcpForwarding yes
    -J 指定多级堡垒机 ssh -L 80:ip:80 -J usr1@jump1 usr2@jump2
    -p 指定 SSH 端口
...
nm|显示库符号 nm | grep '_Z' | c++filt nm | grep 'Swift' | xcrun swift-demangle
...
sudo nvram AutoBoot=%00 禁用mac开盖自动启动(拆后盖会自动启动) %03启用
...
- MacBook Pro A1990 2018 15寸 Display cable
- 丑闻当然被称为Flexgate，得名于Flex显示线缆引发的问题
- https://zh.ifixit.com/Guide/2018+%E5%B9%B4%E5%B8%A6+Touch+Bar+%E7%9A%8415%E5%AF%B8+MacBook+Pro+%E5%B1%8F%E5%B9%95%E6%9B%B4%E6%8D%A2/122939
- 电池放电至25％以下 sudo nvram AutoBoot=%00
- 底部6个螺丝, 开盖手坳处吸起缝隙,用三角片从缝隙滑动到角并转向到侧面中部这里有个钩子(两侧)
- 抬起手指头可以伸进去的高度扶住不要掀起(高度小于1英寸不要拆下钩子)
- 抓住后盖向下拉半厘米左右分离屏幕钩子(先拉一侧再另一侧)
- 装回去钩子,对准屏幕放下盖子向屏幕推,全部对齐后按下去
- FlexGate缺陷的一部分而失效的集成带状电缆 stage lights
- 剥离并取下最靠近电池的主板边缘的大矩形电池板盖 可能要热吹风
- 剥去覆盖电池板数据线接口的任何胶带。
- 使用撬棒轻轻撬起电池板数据线的 ZIF 接口上的锁定盖板
- 将电池板数据线从插槽中滑出，将其断开。沿电缆方向平行于主板滑动
...
- 使用撬棒提起电池电源接口，断开电池
- 使用 T3 Torx 螺丝刀卸下将盖板固定在显示板柔性电缆上的两颗3.5毫米螺丝；移除显示板柔性电缆盖子
- 使用 T3 Torx 螺丝刀移除固定显示板电缆连接器支架的两个 1.7 mm 螺丝；从显示板电缆接口上拆下支架
- 用撬棒直接撬起以断开显示板柔性电缆
- 使用 T3 Torx螺丝刀卸下固定每个盖子的四个2.0 mm 螺丝（总共四个螺丝）移除盖子
- 使用 T5 Torx 螺丝刀拆下固定天线电缆束的单个 2.9 mm 螺丝
- 从插座中垂直向上撬开所有三根天线电缆。
- 将镊子或撬棒的扁平端滑到每根电缆下方，直到它靠近插座，然后轻轻扭动或撬起以断开连接。
- 要重新连接每根电缆，请将接口直接对准其插槽，然后向下按，使其卡入到位
- 使用 T5 Torx 螺丝刀卸下固定显示板的四颗3.3毫米螺丝
- 在重新安装这些螺丝之前，确保显示板柔性电缆位于电路板顶部，而不是位于电路板下方
- 使用 P2 五角螺丝刀卸下固定天线组件的16个1.5 mm 螺丝
- 使用三角撬片撬起天线组件并将其与MacBook Pro分开
- 小心地卸下天线组件，同时将天线电缆束从散热器下方取出
- 在重新组装过程中，轻轻地将电缆束夹在一起，并将其引导到热管下方的正确位置。如果需要，请使用你的工具来指导它，但不要强制它
- 使用 T3 Torx 螺丝刀卸下固定显示屏电缆盖弹簧的四个 3.9 mm 螺丝
- 使用T8 Torx 螺丝刀从每个合页上卸下两个4.9 mm 螺丝（总共四个螺丝）
- 每个合页都有第三个螺丝，你现在应该留在原位
...
- 如果你打算重新使用屏幕，请将其打开并在每个合页正上方的玻璃板上贴一条胶带。这有助于防止玻璃在下一步中被划伤
- 完全打开屏幕，将MacBook Pro放在一边
- 使用T8 Torx螺丝刀卸下剩余的两个4.9 mm 合页螺丝（每个合页一个）
- 最后卸下最上面的螺丝，用一只手稳定 MacBook Pro 的两半，确保它们不会分离并意外掉落
- 牢牢抓住MacBook Pro的屏幕和主体。在这一步中，任何一半都会意外跌落
- 将MacBook Pro 的两半推到一起，以便合页可以从机箱中的凹槽中抬起
- 将MacBook Pro的主体推离你，同时将屏幕拉向你以将其分开
- 移除显示屏
...
- c 指定进程名称
- d 指定文件描述符
- +d 指定目录下一级文件打开情况, 不递归目录
- +D 指定目录下所有文件打开情况, 递归
- i 列出所有网络链接 -i :port -i tcp/udp -i @ip
- /xx/xx/xx 直接查看指定文件打开情况
- -t 只输出进程id
