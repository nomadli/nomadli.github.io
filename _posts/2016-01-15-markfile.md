...
## auto系列
- autoscan 扫描源码生成 configure.scan 重命名 configure.ac
- aclocal 扫描configure.ac生成aclocal.m4 包含自建及三方的宏定义
- autoconf 扫描configure.ac生成configure 遇到不认识的宏去aclocal.m4查
- autoheader 扫描configure.ac生成config.h.in
- automake 扫描Makefile.am生成Makefile.in
- m4 宏工具 autoconf依赖m4展开shell脚本
- libtool是脚本 生成编译器连接器的参数
- autoreconf 自动按合理顺序调用上面的工具
```sh
#configure.ac 建议书写顺序
...
#常见宏
AC_PREREQ               autoconf 版本要求
AC_INIT                 定义软件包信息
AC_CONFIG_SCRDIR        确定源文件存在
AC_CONFIG_HEADER        指定config.h.in生成路径
AC_CONFIG_AUX_DIR       指定辅助脚本文件工具安装目录
AC_CONFIG_MACRO_DIR     宏文件m4存放路径, Makefile.am中 ACLOCAL_AMFLAGS = -I m4路径
AM_INIT_AUTOMAKE        指定需要调用automake
AM_PROG_AR              指定AR路径
AC_PROG_CC              指定C编译器路径
AC_MSG_xxx              AC_MSG_ERROR... 
AC_DISABLE_SHARED       禁用动态库, 用户用--enable-shared覆盖
LT_INIT                 使用libtool, 需要去掉AC_PROG_RANLIB
AC_CANONICAL_HOST       生成host_cpu,host_vendor,host_os
AC_CHECK_HEADERS        检查头文件
AC_DEFINE               符号定义,需要AH_TEMPLATE模板
AC_DEFINE_UNQUOTED      类似AC_DEFINE
AM_CONDITIONAL          生成automake条件宏, 在Makefile.am使用这个条件判断
AC_ARG_ENABLE           增加编译选项
AS_HELP_STRING          格式化字符串
AC_SUBST                创建或给变量赋值
AC_CONFIG_FILES         生成Makefile文件
