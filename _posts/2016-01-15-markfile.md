...
- 指令
        ...
        cmake_minimum_required(VERSION 2.6 [FATAL_ERROR])     设置最低版本
        project(name)           设置工程名
                                会隐式的定义PROJECT_BINARY_DIR和PROJECT_SOURCE_DIR两个变量
                                CMAKE_BINARY_DIR == name_BINARY_DIR ==
                                PROJECT_BINARY_DIR
                                CMAKE_SOURCE_DIR == name_SOURCE_DIR ==
                                PROJECT_SOURCE_DIR                       
        set(xxx yyy)            创建变量xxx,值为yyy
        $ENV{name}              获取环境变量
        set(ENV{name} xxx)      设置环境变量
        add_definitions         添加宏
        add_dependencies        设置target依赖的其他target
        add_executable(name xx.c ..)      编译可执行文件
        add_library(name [SHARED|STATIC|MODULE] [EXCLUDE_FROM_ALL] xx.c ..)      编译静态或动态库
...
```
CMake变量CMAKE_INSTALL_PREFIX用来确定文件安装的根目录。
### 添加测试
在顶层CMakeLists文件的最后
```
