...
- kprobes 内核跟踪
    ...
    ```shell
    #添加一个调用do_sys_open函数前监控,打印参数
    echo 'p:myprobe do_sys_open dfd=%ax filename=%dx flags=%cx mode=+4($stack)' > /sys/kernel/debug/tracing/kprobe_events
    #添加一个调用do_sys_open返回值
    echo 'r:myretprobe do_sys_open $retval' >> /sys/kernel/debug/tracing/kprobe_events
    #启用
    echo 1 > /sys/kernel/debug/tracing/events/kprobes/myprobe/enable
    echo 1 > /sys/kernel/debug/tracing/events/kprobes/myretprobe/enable
    #输出
    tial -f /sys/kernel/debug/tracing/trace
    #关闭监控
    echo > /sys/kernel/debug/tracing/kprobe_events | echo -:myprobe >> kprobe_events
    ```
    ```C
...
```shell
objdump -d /root/test
echo 'p:func_name /root/test/uprobe/uprobe:addr-400000得到的地址' >> /sys/kernel/debug/tracing/uprobe_events
run /root/test
#启动监控
