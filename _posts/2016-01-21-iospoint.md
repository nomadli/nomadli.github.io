...
6.  \_\_attribute\_\_((constructor(num))) 在main函数之前调用的函数,num[101,)表示优先级,数字小优先级高,100以下系统占用。
7.  \_\_attribute\_\_((destructor)) 在进程退出时调用。-Wl,-whole-archive libx.a 链接所有符号否则静态库中将丢失,用atexit函数代替 -Wl, -no-whole-archive
8.  \_\_attribute\_\_((cleanup(函数指针))) 变量名后 在出变量的作用域后执行函数。技巧定义一个未使用的block变量在block里它的释放函数里做释放其他变量:__strong void(^block)(void) \_\_attribute\_\_((cleanup(blockCleanUp),unused)) = ^{};定义一个block变量在出block作用域后调用blockCleanUp,blockCleanUp调用block.
9.  \_\_attribute\_\_((objc_subclassing_restricted)) 在interface前 禁止被继承。
...
15. \_\_attribute\_\_((aligned(8))) //字节对其
16. \_\_attribute\_\_((\_\_packed\_\_)) //紧密排列
17. \_\_attribute\_\_((\_\_transparent_union\_\_)) //可以实现动态类型参数,即将传入参数转为联合体
