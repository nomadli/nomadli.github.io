...
02. watchpoint set v -w read -- address 读监控
03. watchpoint set v -w read_write -- address 读写
04. watchpoint set v -w write -- address 写
05. watchpoint delete num
06. watchpoint modify -c 表达式
06. pro hand -p true -s false SIGABRT/...  允许信号xcode不中断指定信号
07. breakpoint|br  br set -r . -s <PRODUCT_NAME> 在指定项目中正则设置断点
08. br set -n name 在指定名称上设置断点
09. br set -S alignLeftEdges: 指定参数名设置断点
10. br set -l num -f filename
11. br set -a mem_address
12. br set -F functionname
13. br set --func-regex 函数正则
14. br set --shlib xx.dylib --name funcname
15. br set --method|--name "-[xxx xxx]"
16. br set --selector xxx 第OC所有的函数上设置断点
17. br command add 断点序号
18. br set -F funcname 对导出函数设置断点
13. br xx -o condition
14. br xx -i ignone_times
15. br clear -l num -f filename
16. br l 显示所有断点
16. expression|expr  expr $r6=1 设置r6寄存器的值
17. expr $r6 显示r6寄存器值
18. expr xxx = xxx
19. backtrace|bt 打印调用栈
20. frame info 当前帧信息
21. frame select 0 选择栈帧
22. run|r 运行程序
23. process continue|c 继续执行
24. step|s 源码单步并进入
25. stepi|si 指令级别单步并进入
26. next|n 源码单步
27. nexti|ni 指令级单步
28. finish 运行到函数返回
29. return 直接从函数返回
30. thread list 打印线程
31. image list 显示所有库
32. image list -f -o 模块列表
33. image list -f -o xxx 查找SALR偏移
34. image lookup -a 查找原始地址信息
35. image lookup -r -n funcname 查找有调试符号的函数
36. image lookup -r -s funcname 查找无调试符号的函数
37. disassemble -a 地址  反汇编
38. disassemble -A thumbv4t|thumbv5|thumbv5e|thumbv6|thumbv6m|thumbv7|thumbv7f|thumbv7s|thumbv7k|thumbv7m|thumbv7em
39. memory read 起始地址 结束地址 | 寄存器 -outfile filepath --binary -force
40. memory write 寄存器| 地址  数据
41. x address 读取内存显示为字符串
42. register read/x|d 读寄存器显示为十六或十进制
43. register write r9 数据 写寄存器
44. display 表达式 每次单步执行后打印表达式的值
45. po [xx.button allTargets] 找按钮事件函数
```
