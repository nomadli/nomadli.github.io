...
## 值类型
- 左值      在C/C++语言层面可以通过语法正常获取地址的变量
- 右值      在C/C++语言层面,除通过魔数计算外无法正常语法获取地址的值(寄存器变量之间无法获取地址)
- 纯右值    无构造和析构函数C类型
- 将亡值    编译器临时构造立刻销毁的
- 右值引用  对编译器隐藏构造的内存对象(右值)的引用 auto &&t = xxx(); 因为是隐藏的所以移动构造函数首先被命中
- 移动语义  其实是浅拷贝, 只是参数是右值引用(提示编译器命中移动构造函数);std::move(r) = static_cast<xxx &&>(r);

## VC++ 内存布局 MSVC ABI
- __declspec(align(16))
- 虚基类指针vbptr与vptr分离
- vc++将虚基类的偏移量单独用一个额外的指针进行索引;将空子类的虚指针,或者具有与基类相同虚函数接口的派生类的虚指针与虚基类的虚指针进行合并.
...
- STL -> msvcprtd -> 从VS2015(toolset v140) ABI保持向前兼容
- __stdcall和__cdecl不要混用

## GCC Clang 内存布局 Itanium ABI
...
- gcc STL = libstdc++ -> 5.1/7.1/8,1/9.1/11.1 ABI变化(5.1 因C++11的COW string list变化) ->
- clang STL = libc++ libcxx.llvm.org 只用2个ABI版本
- 成员类型大小与#pragma pack的最小值
- rdi来存储this指针; 非虚函数是全局普通函数
- 默认添加6个函数:默认构造函数(空), 默认析构(空), 默认拷贝构造, operator=,移动构造,移动赋值
```C++
class A {
    int ax;
    void AFun3();                       // __ZN7Nomadli1A4AFun3(A*) rdi->this
} a;                                    // int ax
class B: public A {
    int bx;
} b;                                    // int ax
...
class A {
    public:
    virtual ~A() {}
    virtual int f1() const {return ax;}
    virtual void f2() const {}          // offset_to_top: 0 虚表指针地址与对象指针地址的差,A类的虚表在最开始所以=0
    int ax;                             // A RTTI 运行时信息指针
} a;// A.vfptr--------------------------// A::~A()[complete]
    // A.ax                             // A::~A()[deleting]
                                        // int A::f1()
                                        // void A::f2()
class B: public A {
    public:
    virtual void f2() override {}       // offset_to_top: 0 虚表指针地址与对象指针地址的差,B类的虚表在最开始所以=0
    int bx;                             // B RTTI 运行时信息指针
} b;// B.A.vfptr------------------------// B::~B()[complete]
    // A.ax                             // B::~B()[deleting]
    // B.bx                             // int A::f1()
                                        // void B::f2()
//-------------------------------------------------------------------------------
...
class C: public A, public B {
    virtual void f1() {}                // offset_to_top: 0 虚表指针地址与对象指针地址的差,C类的虚表在最开始所以=0
    int cx;                             // C RTTI 运行时信息指针
} c;// C.vfptr--------------------------// C::~C()[complete]
    // A.ax                             // C::~C()[deleting]
    // B.bx                             // void C::f1()
    // C.cx
A *pa = new C;  // pa = &C + 8 编译器跳过 vfptr 因为A类型没有虚表指针
B *pb = new C;  // pb = &C + 16
//-------------------------------------------------------------------------------
class A {
public:
...
public:
    C() {}
    virtual void f2() override {}       // offset_to_top: 0 主虚表指针地址与对象指针地址的差,C类的主虚表在最开始所以=0
    int cx;                             // C 主RTTI 运行时信息指针
}c; // C.A.vfptr------------------------// C::~C()[complete]
    // A.ax                             // C::~C()[deleting]
    // C.B.vfptr-----                   // void A::f1()
    // B.bx          |------------------// offset_to_top: -16 虚表指针地址与对象指针地址的差,C类的次虚表在对象指针后16 == -16
    // C.cx                             // C 次RTTI 运行时信息指针
                                        // C::~C()[complete]
                                        // C::~C()[deleting]
                                        // void C::f2()
                                        // void B::f3()
//-------------------------------------------------------------------------------
//解决菱形继承采用虚继承, 这会导致A不会直接嵌入中间类而是只有虚类表指针
class A {
public:
    virtual ~A() {}
    virtual int f1() const {return ax;}
...
public:
    virtual void f2() override {}       // offset_to_top: 0 虚表指针地址与对象指针地址的差,B类的虚表在最开始所以=0
    int bx;                             // B 主RTTI 运行时信息指针
} b;// B.vfptr--------------------------// B::~B()[complete]
    // B.bx                             // B::~B()[deleting]
    // B.A.vfptr-----                   // int A::f1()
    // B.A.ax       |                   // void B::f2()
    //              |-------------------// offset_to_top: -16
...
public:
    virtual void f2() override {}       // offset_to_top: 0 虚表指针地址与对象指针地址的差,D类的虚表在最开始所以=0
    int dx;                             // D 主RTTI 运行时信息指针
} d; //显示B然后C最后A; D的成员变量会在A和C之间
```
## C++ 11
1. auto
...
14. register 变量可以取地址了
```C
    register int a;     //本来只存在于cpu寄存器 gcc -O3 由于开启了优化 register可能被忽略,由编译器决定
    auto *b = &a;   //一旦取地址 a就会存于内存 C23开始支持auto的
```
...
```c
//__asm__ 或 asm 都可以;__volatile__ 编译器不优化,直接到汇编器;不写__volatile__会优化; 中间参数不可以省略
__asm__ __volatile__("asm code"     ;每句汇编用\n结束; \t 可以使生成的汇编代码格式对齐,不必须
 :output                            ;输出结果 "xx" (var)  xx=r寄存器 m变量 +读写 =只读 +&读写不用输入寄存器 =&.. 
 :input                             ;输入参 "xx" (var/immediate) xx=r寄存器(变量或立即数) m使用变量 i使用立即数
 :changed registers);

__asm__ __volatile__ (              ;这里只有汇编所以可以省略后面所有参数,如果有changed registers那么putput和input都要
    "mov r0, r0 \n\t"
    "mov r1, r1 \n\t"
    "mov r2, r2 \n\t"
);
...
20. clang -target arm-linux-android21 -E demo.c -o demo.i 预处理
21. clang -target arm-linux-android21eabi -S demo.i -o demo.s 生成汇编
22. clang -target arm-linux-android21eabi -S demo.s -o demo.o 生成未地址重定向的二进制
23. clang -target arm-linux-android21eabi -S demo.o -o demo 链接生成可执行文件
24. -fno-elide-constructors  禁止返回值优化
