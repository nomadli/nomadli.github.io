...
- 还原步骤
    ...
    ```
- gitlab备份文件tar 仓库储在tar的repositories/@hashed目录
    - 项目 → ​Settings → General → Advanced → Project ID，记录下项目ID(如79)
    - echo -n "79" | sha256sum | cut -c1-8
    - 结果类似 7902699b, 对应路径中的 79/02/7902699b
    - 项目已删除, tar -tf xx.tar 分别输出删除前备份 和 删除后备份 找大缺失ID
    ```shell
        #!/usr/bin/env bash
        set -e
        printf "\033c"
        ...
        hash=$(echo -n "$1" | sha256sum | cut -c1-64)
        echo "cd /data/gitRepo/repositories/@hashed/${hash:0:2}/${hash:2:2}/${hash}.git"
        echo "/opt/gitlab/embedded/bin/git commit-graph write --reachable"
        echo "/opt/gitlab/embedded/bin/git commit-graph verify"
        echo "/opt/gitlab/embedded/bin/git fsck --full"
        echo "gitlab-rake gitlab:background_jobs:work"
    ```
