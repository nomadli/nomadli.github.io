...
- NTSTATUS DriverEntry(IN PDRIVER_OBJECT driverObject, IN PUNICODE_STRING registryPath)
    - 驱动入口、PDRIVER_OBJECT 驱动结构体, 设置回调函数等. registryPath 注册表信息
- ExInitializeDriverRuntime 驱动初始化运行环境,参数DrvRtPoolNxOptIn启用NX保护策略
- IoAllocateDriverObjectExtension 创建驱动扩展对象内存, 内存前有IO_CLIENT_EXTENSION结构
- KsInitializeDriver 初始化AVStream,如PNP回调、电源管理、设备控制等
