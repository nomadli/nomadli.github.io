...
# 推送git服务器
git remote add origin ssh://git@example.com/group/repo.git
...
- git fetch (name) 远程库同步 --unshallow 将指定depth的库及其它分支都拉取到本地 -v --progress --prune
...
- git stash push pop clear list apply drop --keep-index 临时保存修改
...
- git merge-base has1 has2 两个分支指定commit最近的共同祖先 -is-ancestor 
...
Host * 
  AddKeysToAgent yes
  UseKeychain yes
  IdentityFile /Users/xx/.ssh/nomadli
